...
register(
    id='BruteForceEnvAuth-v0',
    entry_point='BF_GymEnvAuth:BruteForceEnvAuth',
)
...
class BruteForceEnvAuth(gymnasium.Env):
    metadata = {}
    def __init__(self, users, passwords, indexPass_map, max_num_steps=100000, agentId:int = 0):
        super(BruteForceEnvAuth, self).__init__()
        ...
        self.auth =Authenticator(user_file="../Data/100-usernames.txt", password_file="../Data/100-passwords.txt")
    ...
    def step(self, action):
        ...
        if action == 0:
            ...
            with self.lock2:
                response_text, status_code = self.auth.authenticate(username=username, password=password, ip_address=self.ip)
...
if __name__ == "__main__":
    ...
    env = BruteForceEnvAuth(users=agent.users, passwords=agent.passwords, indexPass_map=agent.indexPass_map, http_query=agent.http_query_APP3, agentId=1)
    while True:
        env.step(2);
    state = env.reset()
