...
class IH3Agent:    
    ...
    register(
        ...
        #kwargs={'users': users, 'passwords': passwords, 'indexPass_map': None,'agentId':2}
    )
    def __init__(self):
        manager = Manager()
    ...
    def make_env(self):
        ...
        if self.debug_mode:
            return gymnasium.make('BruteForceEnvAuth-v0', max_episode_steps=100000, disable_env_checker=True, kwargs={'users': agent.users, 'passwords': agent.passwords, 'indexPass_map': agent.indexPass_map, 'agentId':2})
        else:
            return make_vec_env('BruteForceEnvAuth-v0', n_envs=1, seed=0,env_kwargs={'users': agent.users, 'passwords': agent.passwords, 'indexPass_map': agent.indexPass_map, 'agentId':2})
