...
def parse_ossec_message(message):
    ossec_pattern = re.compile(
        ...
        r'(?P<hostname>\S+) ossec: '                 # Hostname followed by 'ossec:'
        r'(?P<message>.+)'                           # Message
    )
    ...
    if match:
        ...
        process = 'ossec'  # Process is always 'ossec'
        full_message = match.group('message')

        # Parse key-value pairs in the message
        message_data = {}
        key_value_pattern = re.compile(r'([\w\s]+?):\s*(.*?)(?=;\s*[\w\s]+?:|$)')
        for kv_match in key_value_pattern.finditer(full_message):
            key = kv_match.group(1).strip().lower().replace(' ', '_')
            value = kv_match.group(2).strip()
            message_data[key] = value

        # Remaining message content
        remaining_message = message_data.get('message', '')
...
def process_parsed_message(parsed_message):
    ...
    if parsed_message['type'] == 'ossec':
        str_to_write = f"OSSEC - Facility: {parsed_message['facility']}, Severity: {parsed_message['severity']}, Timestamp: {parsed_message['timestamp']}, Hostname: {parsed_message['hostname']}, Process: {parsed_message['process']}, Data: {parsed_message['data']}, Message: {parsed_message['message']}"
